...
data Expr = Var String | Lam String Expr | App Expr Expr | Clo Expr Env | V Expr | Zero | Succ Expr deriving (Eq,Show,Read) 
...
lookup' :: Expr -> Env -> Expr
lookup' (Var string) [] = Clo (V (Var string)) []
lookup' (Var string) (mapping:env) | fst mapping == (Var string) = snd mapping
                                   | otherwise = lookup' (Var string) env
...
eval1cbv (Conf (Var string) env1 k) = Conf lamExpr env2 k
    where Clo lamExpr env2 = lookup' (Var string) env1
...
--eval (Lam string exp) = recursiveEval (Conf (Lam string exp) [] [])
--eval _ = error "Input expression must be a lambda expression"
...
testExpr :: Expr
testExpr = (App (Lam "x" (Succ (Var "x"))) (Succ(Succ(Succ(Zero)))))